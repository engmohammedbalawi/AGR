...
@login_required
def printer(request):
    return render(request,'printerss.html',{})

def tableprinter(request):
    printer = Device.objects.all().filter(TypeDevice=2)
    return render(request,'tableprinter.html',{'printer':printer})
...
def welcome(request):
    total_pa = PublicAd.objects.count()
    total_emp = Employe.objects.count()
    total_device = Device.objects.count()
    total_rate1 = Device.objects.filter(rate='1').count()
    total_rate3 = Device.objects.filter(rate='3').count()
    total_rate5 = Device.objects.filter(rate='5').count()
    total_main = Maintainaces.objects.count()
    laptop = Device.objects.filter(TypeDevice=1).count()
    printerr = Device.objects.filter(TypeDevice=2).count()
    desktop = Device.objects.filter(TypeDevice=3).count()
    tablet = Device.objects.filter(TypeDevice=4).count()



    devices_main = Device.objects.filter(maintainaces__isnull=False).distinct().count()
 

    return render(request,'welcome.html',{'total_pa':total_pa,
                                          'total_emp':total_emp,
                                          'total_device':total_device,
                                          'total_rate1':total_rate1,
                                          'total_rate3':total_rate3,
                                          'total_rate5':total_rate5,
                                          'total_main':total_main,
                                          'devices_main':devices_main,
                                          'laptop':laptop,
                                          'printerr':printerr,
                                          'desktop':desktop,
                                          'tablet':tablet
...
def insert(request):
    if request.method == 'POST':
        ...
        print("typedevice_id:", typedevice_id)
        print("ramdevice_id:", ramdevice_id)
        print("employe_id:", employe_id)
        print("storagedevice_id:", storagedevice_id)
        # Convert 'null' to None for ramdevice_id
        if ramdevice_id == 'null':
            ramdevice_id = None
        if storagedevice_id == 'null':
            storagedevice_id = None
        ...
        try:
            ...
            # Fetch the RamDevice object using ramdevice_id (which could be None)
            if ramdevice_id is not None:
                ramdevice = RamDevice.objects.get(id=ramdevice_id)
            else:
                ramdevice = None


            if storagedevice_id is not None:
...
def updatedevice(request, id):
    ...
    if request.method == 'POST':
        ...
        device.screen = request.POST['screen']
        device.gpu = request.POST['gpu']
